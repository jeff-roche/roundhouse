...
fn sealed_floor_denies_ssh_write_even_with_an_explicit_config_allow_rule() {
    ...
    let decision = policy.decide_sealed(&params, &ctx(), Taint::Trusted);
...
fn state_dir_write_is_sealed() {
    ...
    assert_eq!(
        policy.decide_sealed(&params, &c, Taint::Trusted).outcome,
        Outcome::Deny,
...
fn dotfile_write_is_sealed_denied_when_home_is_unset_fail_closed() {
    ...
    let decision = policy.decide_sealed(&params, &c, Taint::Trusted);
    assert_eq!(
        decision.outcome,
        Outcome::Deny,
