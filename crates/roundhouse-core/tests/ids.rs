...
fn ids_are_distinct_types_that_round_trip_through_json() {
    ...
    let task = TaskId::new();
    assert_ne!(session.to_string(), task.to_string(), "ids must not collide by construction");
}
...
fn address_variants_match_spec_shape() {
    ...
    let by_handle = Address::Handle { workspace, name: "reviewer".to_string() };
    let by_team = Address::Team { team };
    let by_role = Address::Role { team, role: "lead".to_string() };
    ...
    for addr in [by_session, by_handle, by_team, by_role, by_human] {
        match addr {
            ...
            Address::Handle { workspace: _, name: _ } => {}
            Address::Team { team: _ } => {}
