...
fn unfocused_session_summaries_coalesce_to_4hz() {
    ...
    let summary = |running: u32| SessionSummary { session_id: "s1".into(), running_tasks: running, blocked: false };
    ...
    assert_eq!(coalescer.offer(summary(2), base + Duration::from_millis(100)), None);
    // A third update past the 250ms window emits again.
    assert_eq!(coalescer.offer(summary(3), base + Duration::from_millis(260)), Some(summary(3)));
}
