...
use roundhouse_store::{open, session_events, spawn_writer, CloseReceipt, EventWriter};
...
async fn admit_task_refuses_while_cancelling_and_after_closed() {
    ...
    // The one durable signal that close() has moved past cancel() and is now blocked on
    // the gated close_session call.
