...
use roundhouse_provider::{
    ...
    Params, Plan, Provider, ProviderError, ProviderExt, ReasoningRequest, RequestCtx, RequestPolicy,
...
static RUNNER: once_cell::sync::Lazy<TaskRunner> = once_cell::sync::Lazy::new(TaskRunner::bootstrap);
    ...
    fn stream_chat<'a>(
        ...
        Box::pin(async move {
            let events = vec![
                ...
                StreamEvent::BlockDelta { index: 0, delta: BlockDelta::Text("Hello".into()) },
...
impl roundhouse_provider::HttpTransport for NoopTransport {
    ...
    ) -> futures::future::BoxFuture<'a, Result<roundhouse_provider::HttpResponseStream, roundhouse_provider::TransportError>>
    {
...
async fn load_task_events(store: &roundhouse_store::StorePool, task_id: TaskId) -> Vec<StoredEvent> {
    ...
    rows.into_iter()
        .map(|(session_id, seq, ts_nanos, task_id_opt, payload, schema_v)| StoredEvent {
            ...
            task_id: task_id_opt.map(|s| {
                roundhouse_core::TaskId::from_uuid(uuid::Uuid::parse_str(&s).expect("valid task_id uuid"))
            }),
...
async fn chat_turn_spawns_one_infer_child_task_and_returns_content() {
    ...
    let ctx = RequestCtx { trace_id: None, transport: Arc::new(NoopTransport), api_key: "test".into() };
    ...
    let task_ids: Vec<(Option<String>,)> = conn
        ...
        .await
        .unwrap();
    assert_eq!(task_ids.len(), 2, "exactly one chat task and one infer task, per S-LOOP-1");
    ...
    assert_eq!(infer_task.parent, Some(chat_task.id), "infer task must be a child of the chat task");
...
async fn thinking_deltas_fold_to_a_thinking_block_with_signature_intact() {
    ...
    let events = vec![
        StreamEvent::BlockStart { index: 0, kind: BlockKind::Thinking },
        StreamEvent::BlockDelta {
            index: 0,
            delta: BlockDelta::Thinking { text: "I should check the file first.".into(), signature: None },
        },
        StreamEvent::BlockDelta {
            index: 0,
            delta: BlockDelta::Thinking { text: String::new(), signature: Some("sig-xyz-exact".into()) },
        },
        StreamEvent::BlockStop { index: 0 },
        StreamEvent::MessageStop,
    ];
    ...
    match &blocks[0] {
        ContentBlock::Thinking { text, signature, redacted } => {
            assert_eq!(text, "I should check the file first.");
            assert_eq!(signature, &Some(Signature("sig-xyz-exact".into())), "signature must survive the fold byte-for-byte");
            assert!(!redacted);
        }
