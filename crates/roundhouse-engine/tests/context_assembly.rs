...
use roundhouse_provider::{ContentBlock, Message, MessageRole as Role, tool_def_from_schema};
...
fn render_order_is_system_then_tools_then_turn() {
    let req = assemble_context("claude-sonnet-5", "You are careful.", &sample_tools(), &sample_turn());

    assert_eq!(req.system[0].text, "You are careful.");
    assert_eq!(req.tools[0].name(), "read");
    assert_eq!(req.messages[0].role, Role::User);
    assert!(req.system[0].cache.is_some(), "system layer gets the first cache breakpoint");
}

#[test]
fn assembly_is_deterministic_across_repeated_calls() {
    let req_a = assemble_context("claude-sonnet-5", "You are careful.", &sample_tools(), &sample_turn());
    let req_b = assemble_context("claude-sonnet-5", "You are careful.", &sample_tools(), &sample_turn());
    ...
    assert_eq!(json_a, json_b, "identical inputs must render byte-identical requests for cache effectiveness");
}
