...
/// §7.7's "plus a global cap" — a single, process-scoped token bucket checked in the
/// same `try_acquire` call as the per-session bucket, at `LocalBus::send`'s
/// chokepoint. It bounds *aggregate* message volume across every session sharing this
/// bus, catching a flood spread across many senders that no single per-session bucket
/// would ever see.
///
...
/// distinct senders this crate expects to share a bus. `GLOBAL_BURST`/
/// `GLOBAL_RATE_PER_MIN` are set to *twice* "every member of a full team bursting
/// simultaneously at full per-session burst/rate" — i.e. `2 * 32 * 10 = 640` burst
/// and `2 * 32 * 20 = 1280`/min sustained. That leaves a single session's entire
/// per-session budget (burst 10, sustained 20/min) at under 1/32 of the global
/// ceiling on either axis, so a lone session can never reach the global cap without
/// every other session on the bus also being simultaneously active near their own
/// limits.
...
impl RateLimiter {
    ...
    pub fn try_acquire(&self, session: SessionId, now: Instant) -> Result<(), BusError> {
        ...
        if g.tokens < 1.0 {
            let retry_after_ms =
                ((1.0 - g.tokens) / self.global_rate_per_sec * 1000.0).ceil() as u64;
