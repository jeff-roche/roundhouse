...
fn raw_update_on_events_is_aborted_by_trigger() {
    ...
    assert!(result.is_err(), "S-LOG-2: UPDATE on events must be aborted by the trigger");
    let msg = result.unwrap_err().to_string();
    assert!(msg.contains("append-only"), "trigger error should explain why: {msg}");
}

#[test]
fn raw_delete_on_events_is_aborted_by_trigger() {
    let conn = seeded_conn();
    let result = conn.execute("DELETE FROM events WHERE session_id = ?1 AND seq = ?2", rusqlite::params!["session-a", 1i64]);
    assert!(result.is_err(), "S-LOG-2: DELETE on events must be aborted by the trigger");
}
...
fn append_still_works_after_trigger_install() {
    ...
    assert_eq!(inserted.unwrap(), 1, "INSERT must remain unaffected by the append-only triggers");
}
