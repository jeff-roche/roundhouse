//! Shared request fixtures for `golden_openai_responses.rs` and
//! `conformance_openai_responses.rs` (REALITY-CORRECTIONS §12e): included by
//! `#[path]` into both integration-test binaries, so there is exactly one
//! definition of each fixture request, never two copies drifting apart. The
//! encoded body a reviewer sees in the golden snapshot is therefore provably
//! the same request the conformance suite replays against a cassette.
#![allow(dead_code)]
...
// A deliberately field-less tool-params struct, so the `schemars`-generated
// `input_schema` has the smallest possible key surface (no per-field
// `properties.*`/`required` keys to enumerate in a conformance mask). A
// plain `//` comment (not `///`) so schemars emits no root-level
// `description` key from a struct doc comment either.
