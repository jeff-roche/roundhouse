...
//!
//! Task 4 lands `profiles/moonshot.toml` plus
//! `testdata/cassettes/moonshot/text.cassette`, so the gate is live from here
//! on: any later task (5-17) that adds a profile without a cassette in the
//! same PR now fails this test immediately, rather than the gate staying a
//! dead, permanently-`#[ignore]`d check through six more profile-adding
//! tasks.
