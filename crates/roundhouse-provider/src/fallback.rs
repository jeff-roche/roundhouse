...
use roundhouse_core::{
    Event, SessionId, TaskError, TaskId, TaskOutput, TaskRunner, Timestamp, Trust, Usage,
};
        ...
        match retry_with_policy(key, breaker, semaphore, |_n| provider.stream_chat(req, ctx)).await
        {
            Ok(stream) => {
                ...
                let event = runner.record_task_completed(
                    ...
                    output,
                    usage,
                    // Internal bookkeeping about which attempt/provider/model
                    // won — generated by this fallback chain itself, not
                    // content that entered context from outside the session.
                    Trust::Trusted,
                    1,
                );
                events.push(event);
