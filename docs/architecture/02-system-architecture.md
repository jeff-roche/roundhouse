...
| `roundhouse-policy` | Permission rule language + evaluation engine | core, config (dependency-free policy-file syntax only; this crate compiles executable rules), store (Task 15: the project-scope trust ledger persists through `roundhouse-store`) |
...
| `roundhouse-engine` | Agent loop, session actor, supervision, context mgmt | core, store, policy, sandbox, provider, bus, net (Task 25: session creation registers the session's egress allowlist with the real `LoopbackProxy` at the same call site it decides isolation tier, so `SessionActor`'s home module depends on `roundhouse-net` directly rather than only transitively), mcp (Phase 7 Task 4, ruling W1-R1: `EngineTaskSpawner` (`src/mcp_spawner.rs`) is the production `impl roundhouse_mcp::executor::TaskSpawner`, and session creation calls `roundhouse_mcp::host::McpHost::start` directly, so this crate needs a real dependency on `roundhouse-mcp`, not just a same-workspace sibling), tools (Phase 7 Task 5, ruling W1-R1: `src/tool_dispatch.rs` dispatches a model-issued `ToolUse` to `roundhouse_tools`' real read/write/edit/find/shell executors — the agent loop's built-in tool arm — so this crate needs a real dependency on `roundhouse-tools`, not just a same-workspace sibling) |
| `roundhouse-config` | Layered config loading (builtin/user/project/workspace scopes per §6.2's precedence ranking), dependency-free policy-file syntax, `SecretRef` types | — (no internal `roundhouse-*` dependency) |
...
| `roundhouse-flow` | Workflow definition + durable execution | core, engine, store, sandbox (Task 14, Phase 7 lane W5: `parse::parse_workflow` runs `serde_yaml`'s untrusted-input deserialization out of process, under `roundhouse-sandbox`'s `bounded_parse::run_bounded_subprocess` CPU/wall-clock/output-size bound, structurally closing a quadratic-cost anchor/alias denial of service that no in-process byte cap could — see `src/parse/mod.rs`'s module doc comment) |
| `roundhouse-sched` | Triggers and scheduling | core, engine, store, bus (Task 1, Phase 5: the `Message` trigger's `Binding` doubles as the addressable recipient for a bus mailbox — Task 4's `bind_message_trigger`/`poll_message_trigger` call the `Bus` trait directly, a deliberate, tracked deviation from this table's original `core, engine, store` row; see `crates/roundhouse-sched/Cargo.toml`'s own comment on the edge) |
| `roundhouse-daemon` | Daemon: wiring, API server, lifecycle — runs as `round daemon` | core, proto, store, policy, sandbox, provider, tools, mcp, acp, bus, engine, flow, sched, config, tui, web (Phase 7 Task 9: `main.rs` mounts `roundhouse_web::build_router` behind a loopback-only `tokio::net::TcpListener`, spawned alongside Task 3's Unix-socket accept loop — `roundhouse-web`'s own module doc named this edge as the crate's one remaining wiring cost, "nothing links it yet"; `roundhouse_web::serve` takes the already-bound listener so `axum` itself stays confined to that crate), net (Phase 7 Task 7: `main.rs` is the one real per-process caller of `LoopbackProxy::new`/`serve` — the daemon-wide egress proxy has to be constructed and bound exactly once, at boot, by *something*, and every session's `session_bootstrap.rs` construction needs to name `EgressPolicy`/`LoopbackProxy` directly to register with it; a direct edge replaces the old transitive-only one) — no `roundhouse-secrets` edge, direct or transitive: `roundhouse-secrets` now depends on `roundhouse-provider` normally (Phase 6 Task 2, see that row above), and `roundhouse-provider` depends back on `roundhouse-secrets` only as a `[dev-dependencies]` entry (excluded from the normal build graph) — neither direction gives `roundhouse-daemon` a path to `roundhouse-secrets`, so the daemon genuinely has none as of this writing |
| `roundhouse-tui` | ratatui client | proto, core (Task 2, Phase 7: `Dashboard::apply` takes a raw `SessionId`/`EventPayload` and reduces `Delta` variants itself now that `ServerMessage` is retired, and every match on `ClientEvent::TaskEvent`'s fields needs those same types — `roundhouse-proto` uses them but does not re-export them, so there is no path to them through the `proto` edge) |
...
| `roundhouse-web` | axum API + embedded web client assets | proto, core, flow, store (three deliberate, tracked deviations from this row's original `proto`-only form, all authorised by the same Phase 5 ruling P9; see `crates/roundhouse-web/Cargo.toml`'s own comments on each edge. **core** (Task 31, Phase 5): §11.3 makes SSE's `Last-Event-ID` carry the `(session_id, seq)` cursor, and `SessionId` is defined in `roundhouse-core` — `roundhouse-proto` uses it but does not re-export it, so there is no path to it through the `proto` edge. **flow** (Task 34, Phase 5): §8.6's Runs inbox query and its fingerprint diffing live in `roundhouse-flow`, which already owns `WorkflowRun`/`Report`/`diff_findings` and already has the `roundhouse-store` edge the `tasks`/`events` tables belong to — so this crate consumes a typed function and writes no SQL and has no `rusqlite` edge of its own. **store** (Task 34): `AppState` carries a `roundhouse_store::StorePool`, the workspace's single pooled-connection constructor, rather than opening a bare `rusqlite::Connection` in here) |
...
>
> **A third new dependency edge added the same way (Phase 7, Task 7):** retiring
> `run_demo_session` as boot behavior and wiring real per-session construction (ruling
> W1-R2 authorizes this row's edit) required `roundhouse-daemon`'s own `main.rs`/
> `session_bootstrap.rs` to depend on `roundhouse-net` directly — `LoopbackProxy::serve`
> runs exactly once, at daemon boot, before any session exists (`roundhouse-engine`'s own
> `create_session_with_egress` doc comment states this), and something in the daemon
> process has to be the one caller that constructs and binds it. `roundhouse-daemon`'s
> row above is amended from "no direct `roundhouse-net` edge of its own" to a real one.
...
| Web LAN token entropy | `getrandom` | Phase 5 (`roundhouse-web`) | The 32 bytes behind §11.3's shared per-device LAN token, straight from the OS CSPRNG. Not `rand`: the lockfile already carries `rand` 0.8, 0.9 and 0.10 (`roundhouse-provider` declares 0.9), so reaching for it means picking a major and defending it; `getrandom` is the layer underneath all three and is the entire surface a token needs. Pinned `=0.3.4`, with the `std` feature — required, not cosmetic: without it `getrandom::Error` implements neither `std::error::Error` nor the OS-error lookup its `Display` uses. |
| Constant-time comparison | `subtle` | Phase 5 (`roundhouse-web`) | Comparing a presented §11.3 LAN token against the stored one. Already resolved in `Cargo.lock` transitively and declared by nobody until now, so the declaration adds no graph node. The alternative — a hand-rolled `fold(0, \|acc, (x, y)\| acc \| (x ^ y)) == 0` — is constant-time only for as long as LLVM chooses not to branch on it, and nothing in that source says it must not. Pinned `=2.6.1`. |
...
| AWS signing | `hmac` + `sha2` + `hex` | Phase 6 (`bedrock-converse` codec); `hex` also Phase 5 (`roundhouse-web`) | SigV4 request signing. `hex` arrived a phase early: `roundhouse-secrets` already declares it at `0.4`, and §11.3's LAN token is stored and presented hex-encoded, so `roundhouse-web` pins the same `=0.4.3` rather than adding a second encoder. |
