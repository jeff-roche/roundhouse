...
| `roundhouse-engine` | Agent loop, session actor, supervision, context mgmt | core, store, policy, sandbox, provider, bus, net (Task 25: session creation registers the session's egress allowlist with the real `LoopbackProxy` at the same call site it decides isolation tier, so `SessionActor`'s home module depends on `roundhouse-net` directly rather than only transitively) |
