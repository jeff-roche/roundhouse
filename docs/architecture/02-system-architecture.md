...
| `roundhouse-web` | axum API + embedded web client assets | proto, core (Task 31, Phase 5: §11.3 makes SSE's `Last-Event-ID` carry the `(session_id, seq)` cursor, and `SessionId` is defined in `roundhouse-core` — `roundhouse-proto` uses it but does not re-export it, so there is no path to it through the `proto` edge. A deliberate, tracked deviation from this row's original `proto`-only form; see `crates/roundhouse-web/Cargo.toml`'s own comment on the edge) |
